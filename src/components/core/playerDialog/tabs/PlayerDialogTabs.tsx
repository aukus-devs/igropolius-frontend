    ...
    <Tabs className="px-5" defaultValue={tabs[0].value}>
      <TabsList className="w-full bg-transparent gap-2">
        {tabs.map(({ name, value }) => (
          <TabsTrigger
            key={value}
            className="text-muted-foreground bg-foreground/20 rounded-lg hover:text-foreground transition-colors data-[state=active]:text-primary-foreground data-[state=active]:bg-primary h-8 font-wide-demi"
            value={value}
          >
            {name}
          </TabsTrigger>
